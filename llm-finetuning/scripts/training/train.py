"""
train.py  –  Supervised fine-tuning script for code-fix datasets with full weight training.
...
Layer freezing
--------------
Pass --freeze_n_layers N to freeze the first N transformer blocks (embeddings
are always frozen unless --train_embeddings is passed).
Pass --train_last_n_layers N to train only the last N layers (freezes all others).
Use --full_weight_training to ensure all parameters are trainable (default: True).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # Training hyper-params (enhanced for full weight training)
...
def ensure_full_weight_training(model):
    """Make sure all parameters are trainable for full weight fine-tuning."""
...
def get_optimal_batch_settings(model_size_gb: float = None):
    """Auto-determine optimal batch size and gradient accumulation based on GPU memory."""
